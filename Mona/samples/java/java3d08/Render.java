	...
	*/
	...
	static final public TLVertex[] TransformAndLighting(Vertex[] vtx)
	{
		...
		for (int i=0;i<vtx.length;i++){
			...
			n.z = n.z * 2 * w - light.z;
			n.Normalize();
			float sw = n.DotProduct(eye);
			int s = (sw>0? (int)(Math.pow(sw,20) * 64 * 0xffff) : 0);

			//頂点フォグ  f = (end - d) / (end - start ）
			float end   =   400.0f;					//フォグ開始距離
			float start =  1800.0f;					//フォグ終了距離
			
			float f = (end - v.Length()) / (end - start);	//正確にフォグ係数計算		
			//float f = (end - v.z) / (end - start);	//高速にフォグ係数計算		
			
			if (f > 0) s += (int)(f * 255)<<16;		//スペキュラーにフォグを加算	
