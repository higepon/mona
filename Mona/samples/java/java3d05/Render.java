...
{	
	...
	*/
	...
	static final public TLVertex[] TransformAndLighting(Vertex[] vtx)
	{
		...
		Vector d = new Vector(1.2f,1.2f,1.2f);//拡散光
		Vector s = new Vector(1,1,1);//スペキュラー色
		...
		int r=0,g=0,b=0,sr=0,sg=0,sb=0;
		Vector n = new Vector();
		...
		for (int i=0;i<vtx.length;i++){
			...
			if (sw>0){
				sw = (float)Math.pow(sw,4);//収束			
				sr = (int)(sw * s.x * 255 * 0xffff);
				sg = (int)(sw * s.y * 255 * 0xffff);
				sb = (int)(sw * s.z * 255 * 0xffff);
			}else{
				sr = 0;
				sg = 0;
				sb = 0;
	...
	static final private void DrawPolygon(TLVertex v1,TLVertex v2,TLVertex v3)
	{
		...
		for (int y=top;y<btm;y++){
			...
			int addsr= (maxsr[y]-minsr[y]) / l;
			int addsg= (maxsg[y]-minsg[y]) / l;
			int addsb= (maxsb[y]-minsb[y]) / l;
	...
	{		
		...
		int addsr= (v2.sr-v1.sr) / l;
		int addsg= (v2.sg-v1.sg) / l;
		int addsb= (v2.sb-v1.sb) / l;
