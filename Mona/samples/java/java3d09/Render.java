...
{	
	...
	static final int STATE_BILINER = 64;
	...
	*/
	...
	static final public void DrawIndexedPrimitive(int type,Vertex[] vertices,int[] indices)
	{
		...
		case PRIMITIVE_POLYGON:
			...
			break;
		case PRIMITIVE_POLYGON | STATE_BILINER:
			for (int i=0;i<indices.length;i+=3){
				DrawFilteredPolygon(tvtx[indices[i]],tvtx[indices[i+1]],tvtx[indices[i+2]]);
			}
			break;
		}
	}
	...
	//--------------------------------------------------------------------
	//ポリゴン描画(Bi-Liner Filtering)
	//
	//--------------------------------------------------------------------
	static final private void DrawFilteredPolygon(TLVertex v1,TLVertex v2,TLVertex v3)
	{		
		if (v1.active + v2.active + v3.active != 3) return;
