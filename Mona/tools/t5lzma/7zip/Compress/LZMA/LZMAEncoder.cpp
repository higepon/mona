...
HRESULT CEncoder::GetOptimum(UInt32 position, UInt32 &backRes, UInt32 &lenRes)
{
  ...
  for(i = 0; i < kNumRepDistances; i++)
  {
    ...
    repLens[i] = _matchFinder->GetMatchLen(0 - 1, reps[i], _matchFinder->GetNumAvailableBytes());
...
HRESULT CEncoder::GetOptimumFast(UInt32 position, UInt32 &backRes, UInt32 &lenRes)
{
  ...
  for(UInt32 i = 0; i < kNumRepDistances; i++)
  {
    ...
    repLens[i] = _matchFinder->GetMatchLen(0 - 1, _repDistances[i], _matchFinder->GetNumAvailableBytes());
    if (i == 0 || repLens[i] > repLens[repMaxIndex])
      repMaxIndex = i;
  }
  if(repLens[repMaxIndex] >= _numFastBytes)
  ...
  if (lenMain >= 2)
  {
    ...
    for(UInt32 i = 0; i < kNumRepDistances; i++)
    {
      ...
      UInt32 repLen = _matchFinder->GetMatchLen(0 - 1, _repDistances[i], _matchFinder->GetNumAvailableBytes());
