...
java::awt::Window::invalidate ()
{
	...
	if (guisvrID == THREAD_UNKNOWN) {
		...
		int _x = 0;
		int _y = 0;
		int _w = _bitmap->Width;
		int _h = _bitmap->Height;
		memcpy(_bitmap->Data, buffer, _w * _h * sizeof(int));
		MonAPI::Message::sendReceive(NULL, guisvrID, MSG_GUISERVER_DRAWWINDOW, 
			_window->Handle, MAKE_DWORD(_x, _y), MAKE_DWORD(_w, _h));
	}
}
...
static java::awt::Window* _handle;
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
		...
		case WM_LBUTTONDOWN:
			if (_handle != NULL)
				_handle->onMouseDown(1, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			break;
		case WM_RBUTTONDOWN:
			if (_handle != NULL)
				_handle->onMouseDown(2, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			break;
		case WM_LBUTTONUP:
			if (_handle != NULL)
				_handle->onMouseUp(1, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			break;
		case WM_RBUTTONUP:
			if (_handle != NULL)
				_handle->onMouseUp(2, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			break;
		case WM_MOUSEMOVE:
			if (_handle != NULL)
				_handle->onMouseMove(wParam & 3, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
			break;
		case WM_KEYDOWN:
			if (wParam == VK_SHIFT)
				shift |= 1;
				...
				shift |= 2;
			if (_handle != NULL)
				_handle->onKeyDown(wParam, shift);
			break;
		case WM_KEYUP:
			if (wParam == VK_SHIFT)
				shift &= ~1;
				...
				shift &= ~2;
			if (_handle != NULL)
				_handle->onKeyUp(wParam, shift);
			break;
		case WM_CHAR:
			if (_handle != NULL)
				_handle->onKeyPress(wParam);
			break;
		case WM_DESTROY:
			alive = false;
			PostQuitMessage(0);
