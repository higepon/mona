...
{
    MessageInfo dest, src;
    src.header = MSG_INTERRUPTED;

    for (;;)
    {
       Message::receive(&dest, &src, Message::equalsHeader);
       if (dest.arg1 == IRQ_NUMBER) break;
    }

//    mm.. danger danger@bochs 2004/12/27
//    while (!interrupt_);
}
