...
#elif defined(WIN32)
#include "nmwin32_a.c"
#elif (defined(OSASK) || defined(MONA) || defined(MONACUI) || defined(PEKOE) || defined(SDL) || defined(NWSOS))
#include "nmport_a.c"
#endif
...
/* static */ WClass *getClass(UtfString className)
	...
	// assign class name
//#if defined(NWSOS)
	// よくわからんがコンパイルエラーになるのでその対策
	wclass->classNameIndex = CONS_nameIndex(wclass, (uint16)(((&wclass->attrib2[2])[0]<<8)|(&wclass->attrib2[2])[1]));
//#else
...
/* static */ Var constantToVar(WClass *wclass, uint16 idx)
	...
	switch (CONS_tag(wclass, idx))
		...
		case CONSTANT_String:
			...
			UtfString st = getUtfString(wclass, stringIndex);// LightCone氏指摘
			v.obj = createStringFromUtf(st);// LightCone氏指摘
			break;
		case CONSTANT_Long:
		case CONSTANT_Double:
		default:
...
#define VALID_OBJ(o) (o > (uint32)FIRST_OBJ && o <= (uint32)FIRST_OBJ + heap.numHandles)
...
/* static */ void executeMethod(WClass *wclass, WClassMethod *method,
	...
	vmStack[vmStackPtr++].pc = NULL;//0;
	vmStack[vmStackPtr++].refValue = NULL;//0;
	vmStack[vmStackPtr++].refValue = NULL;//0;
