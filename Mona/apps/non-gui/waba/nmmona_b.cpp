...
/* static */ dword getRGB16(int r, int g, int b){
/* 24bit -> 16bit */
		...
		for(j = 0; j < 16; j++){
			for(k = 7; k >= 0; k--){
				...
				if(((asciiMap[c][j] >> k) & 0x1) == 1){
					screen.putPixel16(i * 8 + 7 - k + x0 + g_mainWinOffX, j + y0 + g_mainWinOffY, rgb16);
				}else{
...
/* グラフィックボックスに直線を引く */
	...
	if( dx >= dy ) {
		E = -dx;
		for( i = 0; i <= dx; i++ ) {
			screen.putPixel16(x + g_mainWinOffX, y + g_mainWinOffY, rgb16);
			x += sx;
			E += 2 * dy;
			if( E >= 0 ) {
				y += sy;
...
/* WRPファイルをメモリーに読み込む */
	...
	if(syscall_file_open("/APPS/HELLO.WRP", 0, &filesize2) != 0){
		printf("%s\n", errorMessages[3]);
	}else{
		printf("filesize=%dbyte.\n", filesize2);
