...
extern "C" void monapi_set_irq(int irq, int enabled, int auto_ir2)
{
    if (irq >= 0 && irq <= 7)
    {
        if (enabled == 1) outp8(0x21, (inp8(0x21) & ~(1 << irq)));
        else outp8(0x21, (inp8(0x21) | (1 << irq)));
    }
    else if (irq >= 8 && irq <= 15)
    {
        if (enabled == 1) outp8(0xa1, inp8(0xa1) & ~(1 << (irq - 8)));
        else outp8(0xa1, inp8(0xa1) | (1 << (irq - 8)));

        if (enabled == 1 && auto_ir2 == 1)
        {
...
int NE2000::init()
{
    ...
    reti = probe();
    if(reti != 0 ){
        g_console->printf("Does Not Exist Ne2K!!!\n");
        return -1;
    }
