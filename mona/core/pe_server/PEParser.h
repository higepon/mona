...
public:
    ...
    std::map<uint32_t, uint32_t> fixups;
...
public:
    ...
    inline uint32_t get_EntryPoint() {
        if (NULL == this->data) return 0;
        if (this->address)
        {
            return this->address + this->standard->EntryPointRVA;
        }
        else
        {
            return this->specific->ImageBase + this->standard->EntryPointRVA;
        }
    }
