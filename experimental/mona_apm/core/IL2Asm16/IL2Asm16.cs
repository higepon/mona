...
class IL2Asm16
{
	...
	private bool noMain = false;
	private string pre = null, post = null;
	...
	private void InsertFile(StreamWriter sw, string fn)
	{
		sw.WriteLine();
		sw.WriteLine("; --- BEGIN: {0}", fn);
		sw.WriteLine();
	...
	public bool WriteAsm(string asm)
	{
		...
		if (this.pre != null) this.InsertFile(sw, this.pre);
		sw.WriteLine();
		...
		if (this.post != null) this.InsertFile(sw, this.post);
		sw.Close();
		fs.Close();
		return ok;
	}
