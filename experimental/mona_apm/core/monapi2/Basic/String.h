...
///コンストラクタ
	StringData();
	StringData(uint nLength);
///参照カウントアップ。この変数を参照していると教える。
	inline void registerRef()	{m_iRefCount++;}
...
///参照されていれば(Locked())変更には新しいバッファを作る必要がある。
	inline bool isLocked()		{return getRefCount()>1;}
...
///取得
	inline cpchar1	getString()	const{return m_pStringData->m_szData;}			///<文字列を得る。
	inline	operator cpchar1()	const{return getString();}		///<キャスト演算子。
	inline uint	getLength()	const{return m_pStringData->m_nLength;}			///<文字列の長さを得る('\0'は含まない)。
	inline uint	getSize()	const{return m_pStringData->m_nSize;}			///<文字列の領域サイズを得る。
	inline bool	isEmpty()	const{return getLength()==0;}		///<文字列が空かどうか
...
//操作
	...
	void formatV(cpchar1 cszFormat,vapointer vl);			///<formatなどの可変引数ポインタ版
...
protected:
///初期化
	inline void init()	{m_pStringData = NULL;}
///nLengthは格納可能か。
	bool isContainable(uint nLength);
///長さをセットする
	inline void setLength(uint n)		{m_pStringData->m_nLength=n;}
///自分の参照していたm_pStringDataを解放。
	void releaseBuffer();
///getBufferはユーザーが呼び出す、バッファのコピーを作りながらバッファへのアクセスを得るメソッド。
///こっちは直接アドレスを呼び出す。名前が似てるだけで用途は違うと思った方がいい。
	inline pchar1 getBufferDirect()	{return m_pStringData->m_szData;}
///参照へのアクセス。
	inline StringData* getStringData()	const {return m_pStringData;}
...
///コンストラクタとデストラクタ
	String2();									///<コンストラクタ
	String2(int iInitialBufferLength);			///<コンストラクタ
	String2(cpchar1 cszInitialString);			///<コンストラクタ
	String2(const String2& refInitialString);		///<コンストラクタ
	~String2();									///<デストラクタ
...
///取得
	...
	int		findReverse(char1 cFind) const;					///<逆検索。
	int		findReverse(cpchar1 cszFind) const;				///<逆検索。
	void	getLeft(String2* pstrOut,uint nCount) const;		///<左の数文字から新しいString2を構築。
	void	getRight(String2* pstrOut,uint nCount) const;	///<右の数文字から新しいString2を構築。
	void	getMiddle(String2* pstrOut,uint nStart,uint nCount) const;	///<中央の数文字から新しいString2を構築。

	String2	join(cpchar1 csz) const;						///<連結して新しいString2を構築。
	String2	operator+(cpchar1 csz) const;					///<連結して新しいString2を構築。

//操作
	void copy(cpchar1 csz,int iCount=-1);					///<代入。
	String2& operator=(cpchar1 csz);							///<代入。
	void copy(const String2* pstr);							///<代入。
	String2& operator=(const String2& rstr);					///<代入。
	void joinSelf(cpchar1 csz);								///<自分に直接連結。
	String2& operator+=(cpchar1 csz);						///<自分に直接連結。
	void empty();											///<空にする。
	void format(cpchar1 cszFormat,...);						///<sprintf形式で文字列を作る。
	void formatJoinSelf(cpchar1 cszFormat,...);				///<sprintf形式で文字列を作って最後にくっつける。
	bool setAt(uint n,char1 c);								///<指定のインデックスに文字をセット。
...
普段はoperator cpchar1()が効くので
	cpchar1 p = string;
とか書いても自動で正しくString2からcpchar1へのキャストが効き正しい型に変換されますが
可変引数使用時には型の情報が消滅しますから
	StringFn::format(szBuffer,"%s",string->getString());
と明示的に指定してください。
	StringFn::format(szBuffer,"%s",string);
