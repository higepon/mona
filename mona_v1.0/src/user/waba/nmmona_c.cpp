...
static Var MainWinCreate(Var stack[])
	...
	// óÃàÊÇäDêFÇ≈ìhÇËÇ¬Ç‘Ç∑
	//screen.fillRect16(g_mainWinOffX, g_mainWinOffY, g_mainWinWidth, g_mainWinHeight, Color::rgb(192, 192, 192));
	// óÃàÊÇãÛîíÇ≈ñÑÇﬂÇÈÅiå¥énìIÅEÅEÅj
	for(i = g_mainWinOffX/8; i < (g_mainWinOffX + g_mainWinWidth)/8; i++){
		for(j = g_mainWinOffY/16; j < (g_mainWinOffY + g_mainWinHeight)/16 + 1; j++){
			syscall_set_cursor(i,j);
			putCharacter(' ');
		}
	}
	// ògê¸Çï`Ç≠
	my_drawline0(-1, -1, g_mainWinWidth + 1, -1, Color::rgb(0,0,0));
	my_drawline0(-1, -1, -1, g_mainWinHeight + 1, Color::rgb(0,0,0));
	my_drawline0(g_mainWinWidth + 1, -1, g_mainWinWidth + 1, g_mainWinHeight + 1, Color::rgb(0,0,0));
	my_drawline0(-1, g_mainWinHeight + 1, g_mainWinWidth + 1, g_mainWinHeight + 1, Color::rgb(0,0,0));
	v.obj = 0;
	return v;
	}
...
static Var GraphicsSetColor(Var stack[])
	...
	g = stack[2].intValue;
	b = stack[3].intValue;
	WOBJ_GraphicsRGB(gr) = Color::rgb(r & 0xFF, g & 0xFF, b & 0xFF);
	return v;
	}
...
static Var GraphicsDraw(int type, Var stack[])
	...
	switch(type) {
		case GR_FILLRECT:
			...
			//screen.fillRect16(x + g_mainWinOffX, y + g_mainWinOffY, w, h, rgb24);
			break;
			}
		case GR_DRAWLINE:
		case GR_DOTS:
			...
			if (surfaceType == SURF_WINDOW) {
				my_drawline0(x0, y0, x1, y1, rgb24);
