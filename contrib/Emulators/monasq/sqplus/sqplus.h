...
#define _T(n) n
#define SCSNPRINTF monasq_snprintf
#define SCPUTS monasq_printf
...
inline VarRefPtr createVarRef(SquirrelObject & so,const SQChar * scriptVarName) {
  ...
  if (!so.GetUserData(scriptVarTagName,(SQUserPointer *)&pvr)) {
    ...
    if (!so.GetUserData(scriptVarTagName,(SQUserPointer *)&pvr)) return NULL;
#else
...
#define DECLARE_INSTANCE_TYPE_NAME_(TYPE,NAME) namespace SqPlus { \
  ...
  inline void Push(HSQUIRRELVM v,TYPE * value) { \
    if (!value)  sq_pushnull(v); \
      ...
      TRACE(_T("Push(): could not create INSTANCE (check registration name)")); } \
  inline void Push(HSQUIRRELVM v,TYPE & value) { if (!CreateCopyInstance(GetTypeName(value),value)) ; } \
...
#define DECLARE_INSTANCE_TYPE_NAME_(TYPE,NAME) namespace SqPlus { \
  inline const SQChar * GetTypeName(const TYPE & n)            { return _T(#NAME); } \
  inline void Push(HSQUIRRELVM v,TYPE * value)                 { if (!CreateNativeClassInstance(v,GetTypeName(*value),value,0)) ; } \
  inline void Push(HSQUIRRELVM v,TYPE & value)                 { if (!CreateCopyInstance(GetTypeName(value),value)) ; } \
  inline bool	Match(TypeWrapper<TYPE &>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE,false>(v,idx) != NULL; } \
  inline bool	Match(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE,false>(v,idx) != NULL; } \
  inline TYPE & Get(TypeWrapper<TYPE &>,HSQUIRRELVM v,int idx) { return *GetInstance<TYPE,true>(v,idx); } \
...
#if defined( MONASQ )
#define SQPLUS_CHECK_FNCALL(res) if (!SQ_SUCCEEDED(res)) TRACE("SquirrelFunction<> call failed\n")
#else
...
struct SQClassDef {
  ...
  // All the other Squirrel type-masks are passed normally.
  template<typename Func>
  SQClassDef & funcVarArgs(Func pfunc,const SQChar * name,const SQChar * typeMask=_T("*")) {
  ...
  template<typename Func>
  SQClassDef & staticFuncVarArgs(Func pfunc,const SQChar * name,const SQChar * typeMask=_T("*")) {
...
#if defined( MONASQ )
#define SQPLUS_CHECK_GET(res) if (!SQ_SUCCEEDED(res)) 
#else
...
inline bool	Match(TypeWrapper<intptr_t*>,HSQUIRRELVM v,int idx)          { return sq_gettype(v,idx) == OT_USERPOINTER; }
...
inline intptr_t *         Get(TypeWrapper<intptr_t*>,HSQUIRRELVM v,int idx)    { SQUserPointer p; SQPLUS_CHECK_GET(sq_getuserpointer(v,idx,&p)); return (intptr_t*)p; }
